...
- 音频筛选目标是给 Index TTS / Qwen3-TTS 等做音色克隆的单条高质量参考音频，默认要求总时长和有效语音时长都在 6-30 秒内，并过滤静音过多或中间空白过长的音频。
- 语音时长和停顿优先使用 Silero VAD 估计；不可用时回退到幅度切分。排序会额外惩罚感叹、笑声、唱腔符号、长音拖音、结巴和过快/过慢语速，优先选择普通说话风格。
